...
#ifndef AZPLUGINS_PAIR_EVALUATOR_BPBCORE_H_
#define AZPLUGINS_PAIR_EVALUATOR_BPBCORE_H_
...
struct PairParametersBPBCore : public PairParameters
    {
#ifndef __HIPCC__
    PairParametersBPBCore() : epsilon(0), sigma(0) { }

    PairParametersBPBCore(pybind11::dict v, bool managed = false)
        {
 ...
 * Scalar.
 */
class PairEvaluatorBPBCore : public PairEvaluator
    {
    public:
    //! Define the parameter type used by this pair potential evaluator
    typedef PairParametersBPBCore param_type;
     ...
     */
    DEVICE PairEvaluatorBPBCore(Scalar _rsq, Scalar _rcutsq, const param_type& _params)
        : PairEvaluator(_rsq, _rcutsq), epsilon(_params.epsilon), sigma(_params.sigma)
        {
        }
    ...
    DEVICE bool evalForceAndEnergy(Scalar& force_divr, Scalar& pair_eng, bool energy_shift)
        ...
        if (rsq < rcutsq && epsilon != Scalar(0))
            ...
            pair_eng = sigma_over_r6*sigma_over_r6*epsilon;
            force_divr = (Scalar(12.0) * pair_eng) / rsq;
    ...
    static std::string getName()
        {
        return std::string("bpbcore");
        }
#endif

    protected:
    Scalar epsilon; //!< epsilon parameter (energy scale of interaction)
    Scalar sigma; //!< epsilon parameter (energy scale of interaction)
    };
...
#endif // AZPLUGINS_PAIR_EVALUATOR_BPBCORE_H_
